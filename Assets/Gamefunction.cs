...
public class Gamefunction : MonoBehaviour {
	public GameObject Enemy; //宣告物件，名稱Emeny
	public float time; //宣告浮點數，名稱time
	
	public Text ScoreText; //宣告一個ScoreText的text
	public int Score = 0; // 宣告一整數 Score
	public float Difficult = 0; //宣告難度
	public float DifficultSpeed = 0.01f; //宣告難度
	...
	void Update () {
		...
		{
			Vector3 pos = new Vector3(Random.Range(-2.5f,2.5f),5f,0); //宣告位置pos，Random.Range(-2.5f,2.5f)代表X是2.5到-2.5之間隨機
			Instantiate(Enemy,pos,transform.rotation); //產生敵人
			time = 0f; //時間歸零
			Difficult += 0.1f;
		}
		
		if(Difficult>1f && IsPlaying == true) //增加難度
		{
			DifficultSpeed+=0.01f;
			Difficult = 0f; //難度值歸零
		}
	}
